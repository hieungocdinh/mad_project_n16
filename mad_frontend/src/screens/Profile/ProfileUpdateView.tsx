...
type ProfileUpdateViewProps = {
    navigation: NativeStackNavigationProp<RootStackParamList, 'ProfileUpdateView'>;
    route: RouteProp<RootStackParamList, 'ProfileUpdateView'>;
};

const ProfileUpdateView: React.FC<ProfileUpdateViewProps> = ({ route, navigation }) => {
    const { t } = useTranslation();
    const { profileId, isFirstLogin } = route.params;
    ...
    const fetchProfileDetail = async () => {
        try {
            ...
            setGender((data.gender || 'male').toLowerCase());
            setBirthDate(data.birthDate ? new Date(data.birthDate) : null);
            setDeathDate(data.deathDate ? new Date(data.deathDate) : null);
    ...
    const handleSave = async () => {
        ...
        if (profileData && profileData.avatarUrl !== avatarUrl) {
            try {
        ...
        const profileRequest: ProfileRequest = {
            ...
            biography,
            address,
            ...(isFirstLogin === true && { profileSetting: true }),
        };

        try {
            const response = await profileApi.updateProfile(profileId, profileRequest);
